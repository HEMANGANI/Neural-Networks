...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        positional_encode = self.positional_encoding(embed)
        project = self.visual_projection(features).unsqueeze(1)
        tgt_mask = torch.tril(torch.ones(T, T, device=positional_encode.device, dtype=positional_encode.dtype))
        features = self.transformer(tgt=positional_encode, memory=project, tgt_mask=tgt_mask)
