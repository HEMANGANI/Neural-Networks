...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    grads['W2'] = hl.T.dot(softmax_probs)
    grads['W2'] /= num_train
    grads['W2'] += 2 * reg * W2
    grads['b2'] = np.sum(softmax_probs, axis = 0, keepdims=1)/N

    dh = softmax_probs.dot(W2.T)
    dh[hl<=0] = 0
    grads['W1'] = X.T.dot(dh)
    grads['W1'] /= num_train
    grads['W1'] += 2 * reg * W1
    grads['b1'] = np.sum(dh, axis = 0, keepdims=1)/N
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
  ...
  def predict(self, X):
    ...
    ###########################################################################
    hl = np.maximum(0, X.dot(self.params['W1']) + self.params['b1'])
    scores = hl.dot(self.params['W2']) + self.params['b2']
